...
// We can't read border-radius-top-[left/right], so just hardcode Adwaita's
// value.
constexpr int kAdwaitaBorderRadius = 8;

// The border color is encoded in box-shadow, which also cannot be read, so
// that needs to be hardcoded as well. These colors are taken from:
// https://gitlab.gnome.org/GNOME/gtk/-/blob/3.24.29/gtk/theme/Adwaita/_common.scss#L4557-4560
// Important note: the "transparentize" function returns the color with an alpha
// of 1 - its argument, not the argument itself. That is why we use "1 -" below.
constexpr float kAdwaitaLightBorderAlpha = 1 - 0.77;
constexpr float kAdwaitaDarkBorderAlpha = 1 - 0.25;
constexpr SkColor kAdwaitaLightBorderColor =
    SkColorSetA(SK_ColorBLACK, 255 * kAdwaitaLightBorderAlpha);
constexpr SkColor kAdwaitaDarkBorderColor =
    SkColorSetA(SK_ColorBLACK, 255 * kAdwaitaDarkBorderAlpha);

constexpr int kBorderInset = 1;

constexpr int kResizeInsideBoundsSize = 5;
constexpr int kResizeAreaSize = kResizeInsideBoundsSize + kBorderInset;
...
int ClientFrameViewLinux::ResizingBorderHitTest(const gfx::Point& point) {
  return ResizingBorderHitTestImpl(point, kResizeAreaSize);
}
...
void ClientFrameViewLinux::GetWindowMask(const gfx::Size& size,
                                         SkPath* window_mask) {
  window_mask->addRRect(GetRoundedRectForBounds(gfx::RectF(bounds())));
}
...
void ClientFrameViewLinux::PaintBorder(gfx::Canvas* canvas) {
  // Note that View has integrated border functionality. However, as the
  // built-in border tools do not support rounded borders on only the top of the
  // window, it's easier to just draw the entire border ourselves.
  ...
  flags.setColor(GetNativeTheme()->ShouldUseDarkColors()
                     ? kAdwaitaDarkBorderColor
                     : kAdwaitaLightBorderColor);
...
gfx::Insets ClientFrameViewLinux::GetBorderDecorationInsets() const {
  return gfx::Insets(kBorderInset);
}
